...
menu_bg = pygame.Color(0,0,0)
game_bg = pygame.Color(15,50,15)
scoreCol = pygame.Color(150,250,150)
white = pygame.Color(205,205,205)
red = pygame.Color(252,3,36)
green = pygame.Color(140,252,3)
dark_green = pygame.Color(54,148,0)
...
difficulty = "HARD" # EASY, NORMAL, HARD, INSANE, UNFAIR, IMPOSSIBLE, NIGHTMARE, HELL
...
if sFruit_pos == None:
    fruit_position = [random.randrange(1, int((window_x//10))) * 10,
                    random.randrange(1, int((window_y//10))) * 10]
else:
...
def show_score(choice, color, font, size):
    ...
    # score_surface
    score_surface = score_font.render('Score: ' + str(score), True, scoreCol)
    
    diff_surface = score_font.render('Difficulty: ' + str(difficulty[0:1]) + str(difficulty[1:len(difficulty)]).lower() + f" ({score_increase})", True, scoreCol)
    ...
    diff_rect.midtop = (int(window_x) / 2, 0)
...
def display_different_diff():
    ...
    # setting position of the text
    game_over_rect.midtop = (int(window_x) / 2, int(window_y) / 4)
    game_over_rect2.midtop = (int(window_x) / 2, int(window_y) / 4 + 15)
...
def display_start_menu():
    ...
    enter_to_play_surface = my_font2.render(
        '> Press enter to play! <', True, red)
     
    # create a rectangular object for the text
    # surface object
    game_over_rect = game_over_surface.get_rect()
    game_over_rect2 = game_over_surface2.get_rect()
    enter_to_play_rect = enter_to_play_surface.get_rect()
     
    # setting position of the text
    game_over_rect.midtop = (int(window_x) / 2, int(window_y) / 4)
    game_over_rect2.midtop = (int(window_x) / 2, int(window_y) / 4 + 35)
    enter_to_play_rect.midtop = (int(window_x) / 2, int(window_y) / 4 + (35 * 3))
     
    # blit will draw the text on screen
    game_window.blit(game_over_surface, game_over_rect)
    game_window.blit(game_over_surface2, game_over_rect2)
    game_window.blit(enter_to_play_surface, enter_to_play_rect)
...
while (not playingNot):
    ...
    if not fruit_spawn:
        fruit_position = [random.randrange(1, (int(int(window_x) // 10))) * 10,
                          random.randrange(1, (int(int(window_y) // 10))) * 10]
         
    fruit_spawn = True
    game_window.fill(game_bg)
    ...
    for pos in snake_body:
        ...
        if (i % 2 == 0): 
            ...
            pygame.draw.rect(game_window, dark_green,
                         pygame.Rect(pos[0], pos[1], 10, 10))
    pygame.draw.rect(game_window, red, pygame.Rect(
        fruit_position[0], fruit_position[1], 10, 10))
 
    # Game Over conditions
    if snake_position[0] < 0 or snake_position[0] > int(window_x-10):
        game_over()
    if snake_position[1] < 0 or snake_position[1] > int(window_y-10):
        game_over()
